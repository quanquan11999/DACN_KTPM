...
class OrderService {
  ...
  async addToOrder(payload) {
    ...
    // Kiểm tra số lượng món ăn trong đơn hàng
    for (const product of order.products) {
      ...
      // Kiểm tra xem số lượng món ăn đặt hàng có lớn hơn số lượng còn lại trong quán không
      if (p.quantity_remain < product.quantity_sale) {
        // Nếu số lượng đặt hàng vượt quá số lượng còn trong quán, không thực hiện đặt hàng
        return null;
      }
    }
    //return await this.Order.insertOne(order);
    // Nếu không có món ăn nào vượt quá số lượng trong quán, thêm đơn hàng vào cơ sở dữ liệu
  ...
  async updateOrderStatus(id, payload) {
    ...
    if (payload.status === 2) {
      for (const product of result.products) {
        ...
        const productUpdate = {
          $inc: {
            quantity_sale: +product.quantity_sale, // Thay đổi số lượng đã bán của món ăn
            quantity_remain: -product.quantity_sale, // Giảm số lượng còn lại của món ăn
          },
        };
