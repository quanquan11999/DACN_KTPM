...
class CustomerService {
  ...
  async addToCart(customerId, payload) {
    ...
    // Tìm kiếm món ăn trong giỏ hàng của khách hàng
    const existingProduct = await this.Customer.findOne({
      _id: new ObjectId(customerId),
      "cart.product_id": payload._id,
    });
    // Nếu món ăn đã tồn tại trong giỏ hàng, cập nhật số lượng sản phẩm và tính lại tổng giá trị
    if (existingProduct) {
      ...
      // Nếu món ăn chưa có trong giỏ hàng, thêm sản phẩm vào giỏ hàng
      const result = await this.Customer.updateOne(
        { _id: new ObjectId(customerId) },
        { $push: { cart: cart } }
      );
  ...
  //Xóa món ăn chỉ định trong giỏ hàng
